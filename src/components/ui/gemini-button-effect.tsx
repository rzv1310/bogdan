...
export const GeminiButtonEffect: React.FC<GeminiButtonEffectProps> = ({ className, children }) => {
  ...
  const p1 = useTransform(scrollYProgress, [0, 1], [0.1, 1.15]);
  const p2 = useTransform(scrollYProgress, [0, 1], [0.05, 1.1]);
  const p3 = useTransform(scrollYProgress, [0, 1], [0, 1.05]);

  return (
    <div ref={ref} className={cn("relative inline-block", className)}>
      {/* SVG aura behind the button - does not affect layout */}
      <motion.svg
        className="pointer-events-none absolute left-1/2 top-1/2 -z-10 -translate-x-1/2 -translate-y-1/2 w-[720px] h-[260px]"
        viewBox="0 0 720 260"
